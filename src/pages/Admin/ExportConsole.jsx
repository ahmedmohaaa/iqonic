...
/* ═══════════════════════════════════════════════════════════
   فلترة التاريخ داخل الفرونت إند
   تُستخدم للتقارير التي لا يدعم الباك إند فلترة التاريخ فيها
═══════════════════════════════════════════════════════════ */
const toDateKey = (value) => {
  ...
  // لو التاريخ بصيغة YYYY-MM-DD أو ISO، نأخذ أول 10 أرقام مباشرة
...
const getRowDateKey = (row, dateFields = []) => {
  if (!row || !Array.isArray(dateFields) || !dateFields.length) return null;

  for (const field of dateFields) {
    const value = toDateKey(row?.[field]);
...
const filterRowsByDateRange = (rows, dateFields, dateFrom, dateTo) => {
  ...
  if (!Array.isArray(dateFields) || !dateFields.length) return rows;

  return rows.filter((row) => {
    const rowDate = getRowDateKey(row, dateFields);

    // لو الصف لا يحتوي على تاريخ، لا يمكن تقييمه داخل النطاق
...
export default function ExportConsole() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      /*
        تقرير المشاريع فقط هو الذي يدعم فلترة التاريخ من الباك إند.
        لذلك نرسل date_from و date_to فقط عند اختيار المشاريع.
      */
      if (cfg.backendDateFilter) {
      ...
      /*
        باقي التقارير يتم فلترة التاريخ فيها داخل الفرونت إند
        بعد استلام البيانات من الباك إند.
      */
      if (!cfg.backendDateFilter) {
        list = filterRowsByDateRange(list, cfg.dateFields, dateFrom, dateTo);
      }
          ...
          <div className="xc-formats">
            {FORMATS.map((f) => (
              ...
              >
                ...
                {lastFormat === f.id && status === EXPORT_STATUS.DONE && (
                  <Check size={16} className="xc-fmt-check" />
                )}
              </button>
            ))}
          </div>
                ...
                <table className="xc-table">
                  ...
                  <tbody>
                    {preview.map((r, ri) => (
                      <tr key={ri}>
                        {activeCols.map((c) => (
                          <td key={colId(c)}>{colValue(c, r)}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
...
.xc-press-dot{
  ...
  box-shadow:inset 0 0 0 2px #94a3b8; animation:xc-press 2.4s ease-in-out infinite;
}

.xc-press-dot:nth-child(2){ animation-delay:.3s; }
.xc-press-dot:nth-child(3){ animation-delay:.6s; }

@keyframes xc-press{
  0%,100%{ background:#cbd5e1; }
  50%{ background:var(--amber); box-shadow:0 0 12px rgba(217,119,6,.4); }
}
...
.xc-title{
  ...
  font-size:clamp(28px,5vw,46px); font-weight:700; line-height:1.02;
...
.xc-status{
  ...
  border-radius:11px; border:1px solid var(--line); background:rgba(0,0,0,.02);
...
.xc-status--done .xc-status-ico{ color:var(--emerald); }
.xc-status--error .xc-status-ico{ color:var(--rose); }
...
.xc-spin{ animation:xc-spin .8s linear infinite; }
...
.xc-panel{
  background:linear-gradient(180deg,rgba(0,0,0,.01),transparent),var(--surface);
...
.xc-src{
  ...
  color:#334155; font-family:inherit; font-size:13px; cursor:pointer;
  transition:.2s; text-align:start;
}
...
.xc-src.on{
  background:rgba(2,132,199,.06); border-color:rgba(2,132,199,.3);
...
.xc-dates input{
  ...
  border-radius:9px; padding:8px 10px; color:var(--paper);
  font-family:inherit; font-size:12px; transition:.2s;
  box-shadow:inset 0 1px 2px rgba(0,0,0,.02); outline:none;
}
...
.xc-apply{
  ...
  border:1px solid var(--line); background:#ffffff; color:var(--paper);
  font-family:inherit; font-size:13px; font-weight:600; cursor:pointer;
  transition:.2s; box-shadow:0 1px 2px rgba(0,0,0,.03);
}

.xc-apply:hover{ border-color:var(--emerald); color:var(--emerald); }
.xc-apply:disabled{ opacity:.6; background:#f1f5f9; }
...
.xc-col-box{
  width:15px; height:15px; border-radius:4px; border:1px solid #cbd5e1;
  display:grid; place-items:center; font-size:10px; color:var(--emerald);
}
...
.xc-fmt{
  ...
  background:linear-gradient(180deg,rgba(0,0,0,.01),transparent),var(--surface);
  cursor:pointer; font-family:inherit; text-align:start; min-width:0;
...
.xc-fmt--rose .xc-fmt-ico{ background:rgba(220,38,38,.1); color:var(--rose); }
.xc-fmt--rose:hover{ border-color:rgba(220,38,38,.4); box-shadow:0 12px 24px -12px rgba(220,38,38,.3); }

.xc-fmt--emerald .xc-fmt-ico{ background:rgba(5,150,105,.1); color:var(--emerald); }
.xc-fmt--emerald:hover{ border-color:rgba(5,150,105,.4); box-shadow:0 12px 24px -12px rgba(5,150,105,.3); }

.xc-fmt--sky .xc-fmt-ico{ background:rgba(2,132,199,.1); color:var(--sky); }
.xc-fmt--sky:hover{ border-color:rgba(2,132,199,.4); box-shadow:0 12px 24px -12px rgba(2,132,199,.3); }

.xc-fmt--amber .xc-fmt-ico{ background:rgba(217,119,6,.1); color:var(--amber); }
.xc-fmt--amber:hover{ border-color:rgba(217,119,6,.4); box-shadow:0 12px 24px -12px rgba(217,119,6,.3); }
...
.xc-preview{
  ...
  box-shadow:0 4px 6px -1px rgba(0,0,0,.02); min-width:0; overflow:hidden;
}
...
.xc-table th{
  text-align:start; padding:9px 12px; font-size:10px;
  letter-spacing:.08em; text-transform:uppercase; color:var(--muted);
  border-bottom:1px solid var(--line); white-space:nowrap;
}

.xc-table td{
  padding:9px 12px; border-bottom:1px solid var(--line); color:#334155;
  white-space:nowrap; max-width:240px; overflow:hidden; text-overflow:ellipsis;
}

.xc-table tbody tr{ transition:background .18s; }
.xc-table tbody tr:hover{ background:rgba(2,132,199,.04); }

.xc-empty{
  display:flex; align-items:center; justify-content:center; gap:8px;
  padding:34px; color:var(--muted); font-size:13px;
}
`;