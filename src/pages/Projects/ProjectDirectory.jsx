...
import { getProjectCards } from '../../api/services/projects';
import { Link } from 'react-router-dom';
import { Calendar, AlertCircle, Plus, Clock, Flag, Edit, Eye } from 'lucide-react';

const ProjectDirectory = ({ userRole = 'Manager' }) => {
  // ملاحظة: يمكنك استبدال userRole بالمتغير القادم من الـ Context الخاص بالمستخدم الحالي لديك
  ...
  // دالة لترتيب المشاريع حسب النشاط ثم الأولوية
  const getSortedProjects = (projectsList) => {
    const priorityWeights = { URGENT: 1, HIGH: 2, MEDIUM: 3, LOW: 4 };

    return [...projectsList].sort((a, b) => {
      // 1. الأولوية لحالة المشروع (النشط يعرض أولاً والمغلق في النهاية)
      if (a.is_active && !b.is_active) return -1;
      if (!a.is_active && b.is_active) return 1;

      // 2. إذا كانت المشاريع متساوية في الحالة (كلاهما نشط أو كلاهما مغلق)، نرتب حسب الأولوية
  ...
  // التحقق من صلاحيات المستخدم الحالي لعرض الأزرار الثلاثة المتقدمة
  const allowedRoles = ['Manager', 'Assistant', 'Secretary', 'مدير', 'مساعد', 'سكرتيرة'];
  const hasActionPermissions = allowedRoles.includes(userRole);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/projects/create" className="bg-primary text-white px-4 py-2 rounded-lg bg-blue-800 flex items-center hover:bg-blue-900 transition">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {sortedProjects.map(project => (
          <div key={project.id} className="bg-white rounded-lg shadow shadow-lg transition flex flex-col border-t-4 border-primary hover:shadow-xl">
            
            {/* الجزء الخاص بعرض بيانات المشروع - قابل للضغط */}
            <Link to={`/projects/${project.id}`} className="p-5 block grow">
              <div className="flex justify-between items-start mb-3">
                ...
                <span className={`px-2 py-1 text-xs rounded-full ${project.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
              ...
              {project.priority && project.is_active && (
                <div className={`mt-3 flex items-center text-sm font-semibold ${
                  ...
                  <AlertCircle size5={16} className="mr-1" /> {project.priority} Priority
                </div>
              )}
            </Link>

            {/* الفوتر الخاص بالأزرار في أسفل الكارت */}
            <div className="border-t border-gray-100 p-3 bg-gray-50 flex flex-col gap-2 rounded-b-lg">
              
              {/* زر Project Details (يراه الجميع ويؤدي إلى /projects/:id) */}
              <Link 
              ...
              {/* الأزرار الإضافية للمدير والمساعد والسكرتيرة فقط */}
