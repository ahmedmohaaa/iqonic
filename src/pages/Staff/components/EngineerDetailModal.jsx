...
.skm-mask{ position:fixed; inset:0; z-index:60; display:grid; place-items:center; padding:18px; background:rgba(15,23,42,.40); backdrop-filter:blur(4px); opacity:0; transition:opacity .25s; }
.skm-mask.in{ opacity:1; }
.skm{ position:relative; width:min(720px,100%); max-height:90vh; overflow-y:auto; overflow-x:hidden; background:linear-gradient(180deg,#ffffff,#f8fafc); border:1px solid #e2e8f0; border-radius:20px; color:#0f172a; font-family:'IBM Plex Sans Arabic','Space Grotesk',sans-serif; transform:scale(.96) translateY(10px); transition:transform .3s cubic-bezier(.2,.8,.2,1); box-shadow:0 40px 80px -30px rgba(0,0,0,.8); }
...
.skm-head{ display:flex; justify-content:space-between; align-items:flex-start; gap:14px; padding:22px 24px 18px; border-bottom:1px solid #e2e8f0; }
.skm-id{ display:flex; align-items:center; gap:13px; min-width:0; }
.skm-av{ width:52px; height:52px; flex:none; border-radius:14px; display:grid; place-items:center; background:linear-gradient(145deg,#0284c7,#2f8fc0); color:#04121c; font-family:'Space Grotesk'; font-weight:700; font-size:18px; }
.skm-id-txt h2{ font-family:'Space Grotesk','IBM Plex Sans Arabic'; font-size:21px; font-weight:700; margin:0; line-height:1.15; }
.skm-id-txt p{ display:flex; align-items:center; gap:7px; flex-wrap:wrap; margin:4px 0 0; font-size:12px; color:#64748b; }
.skm-role{ color:#0284c7; font-weight:600; } .skm-sep{ opacity:.4; }
.skm-multi{ font-size:9px; font-weight:700; background:rgba(230,171,76,.16); color:#d97706; padding:2px 7px; border-radius:99px; }
...
.skm-ring-fg{ fill:none; stroke:#059669; stroke-width:5; stroke-linecap:round; transition:stroke-dasharray 1s cubic-bezier(.2,.7,.2,1); }
...
.skm-ring-t i{ font-style:normal; font-size:9px; color:#64748b; }
.skm-x{ width:34px; height:34px; border-radius:9px; display:grid; place-items:center; background:rgba(255,255,255,.05); border:1px solid #e2e8f0; color:#64748b; cursor:pointer; transition:.2s; }
.skm-x:hover{ color:#fff; background:rgba(227,112,126,.15); border-color:rgba(227,112,126,.4); }
.skm-load{ display:flex; align-items:center; justify-content:center; gap:10px; padding:60px; color:#64748b; }
...
.skm-stat{ display:flex; flex-direction:column; align-items:center; gap:6px; padding:12px 6px; border:1px solid #e2e8f0; border-radius:12px; background:rgba(255,255,255,.02); transition:transform .25s, border-color .25s; }
...
.skm-stat-l{ font-size:9.5px; color:#64748b; text-align:center; }
.skm-stat.t-sky .skm-stat-ic{ background:rgba(92,198,239,.14); color:#0284c7; } .skm-stat.t-sky .skm-stat-v{ color:#0284c7; }
.skm-stat.t-emerald .skm-stat-ic{ background:rgba(63,178,134,.14); color:#059669; } .skm-stat.t-emerald .skm-stat-v{ color:#059669; }
.skm-stat.t-amber .skm-stat-ic{ background:rgba(230,171,76,.14); color:#d97706; } .skm-stat.t-amber .skm-stat-v{ color:#d97706; }
.skm-stat.t-rose .skm-stat-ic{ background:rgba(227,112,126,.14); color:#dc2626; } .skm-stat.t-rose .skm-stat-v{ color:#dc2626; }
.skm-stat.t-violet .skm-stat-ic{ background:rgba(161,140,242,.14); color:#a18cf2; } .skm-stat.t-violet .skm-stat-v{ color:#a18cf2; }
.skm-stat.t-slate .skm-stat-ic{ background:rgba(255,255,255,.06); color:#64748b; } .skm-stat.t-slate .skm-stat-v{ color:#334155; }
...
.skm-prog-h{ display:flex; justify-content:space-between; font-size:12px; color:#64748b; margin-bottom:7px; }
.skm-prog-v{ font-family:'JetBrains Mono'; font-weight:700; color:#059669; }
.skm-prog-bar{ height:9px; border-radius:99px; background:rgba(255,255,255,.07); overflow:hidden; }
.skm-prog-bar span{ display:block; height:100%; border-radius:99px; background:linear-gradient(90deg,#0284c7,#059669); position:relative; transition:width 1s cubic-bezier(.2,.7,.2,1); }
...
.skm-tabs{ display:flex; gap:6px; padding:16px 24px 0; border-bottom:1px solid #e2e8f0; }
...
.skm-item{ display:flex; align-items:center; gap:12px; padding:11px 13px; border:1px solid #e2e8f0; border-inline-start-width:3px; border-radius:11px; background:rgba(255,255,255,.02); opacity:0; transform:translateX(-8px); animation:skm-rise .4s ease forwards; animation-delay:calc(var(--i) * 45ms); transition:border-color .25s, background .25s, transform .25s; }
@keyframes skm-rise{ to{ opacity:1; transform:none; } }
.skm-item:hover{ background:rgba(255,255,255,.04); transform:translateX(3px); }
.skm-item.t-emerald{ border-inline-start-color:#059669; } .skm-item.t-sky{ border-inline-start-color:#0284c7; }
.skm-item.t-amber{ border-inline-start-color:#d97706; } .skm-item.t-rose{ border-inline-start-color:#dc2626; } .skm-item.t-slate{ border-inline-start-color:#94a3b8; }
.skm-item-dot{ width:8px; height:8px; border-radius:50%; flex:none; background:#94a3b8; }
.skm-item.t-emerald .skm-item-dot{ background:#059669; } .skm-item.t-sky .skm-item-dot{ background:#0284c7; }
.skm-item.t-amber .skm-item-dot{ background:#d97706; } .skm-item.t-rose .skm-item-dot{ background:#dc2626; }
.skm-item-main{ flex:1; min-width:0; display:flex; flex-direction:column; gap:2px; }
.skm-item-title{ font-size:13px; font-weight:600; color:#0f172a; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.skm-item-sub{ font-size:10.5px; color:#64748b; font-family:'JetBrains Mono'; }
.skm-item-right{ display:flex; flex-direction:column; align-items:flex-end; gap:4px; flex:none; }
.skm-item-pct{ font-family:'JetBrains Mono'; font-weight:700; font-size:13px; color:#0284c7; }
.skm-item-date{ display:inline-flex; align-items:center; gap:4px; font-family:'JetBrains Mono'; font-size:10px; color:#64748b; }
